...
def makegrid(lat, lon, z_sta, stn, t, ssp_dir):
    ...
    Args:
        ...
        ssp_dir (str): ???
    """
    # Path for Levitus database
    module_path = Path(__file__).parent
    levitus_path = module_path.joinpath("data", "ocean_profiles", "ssp_09")
    ...
    if not os.path.exists(ssp_stock_fname):
      ...
      if not os.path.exists(ssp_user_fname):
        ...
        ssp, z = get_lev_based_ssp(lat, lon, month, levitus_path,
                                   'SSP_' + stn + '.txt', ssp_dir)
